<template>
  <div>
    ...
    <van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad">
      <div  :class="[index !== (listData.length-1)?'bottom_line':'','activity_card']" v-for="item in listData" :key="item.id" @click="goDetail(item.id)">
        <van-row>
          <van-col span="9">
          ...
          <van-col span="15" class="activity_card_right">
            <div class="activity_title activity_card_padding" v-text="item.title"></div>
            ...
            <div class="activity_card_padding">
              ...
              <van-tag round plain color="#f5b064" v-if="item.volunteers_count === item.limit">已满</van-tag>
            </div>
          </van-col>
        </van-row>
      </div>
